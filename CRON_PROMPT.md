...
7. 글 구조는 반드시 포함한다.
   ...
   - FAQ 뒤에는 사람이 읽는 흐름을 위해 반드시 아래 순서로 배치한다:
     1. 결론 한 문장
     2. 전문용어 쉽게 풀어쓰기: 일반인이 낯설 수 있는 의학·뇌과학·심리학·통계 용어 5~8개를 쉬운 한국어로 설명한다. 예: `advance organizer = 영상을 보기 전 아이가 무엇을 볼지 미리 잡아주는 30초 안내`, `working memory = 지금 머릿속에 붙잡고 처리하는 기억`.
     3. 연구 핵심 수치 표
     4. 출처와 참고 근거
     5. 이 글과 함께 읽어보세요(사이트 layout의 related-reading 블록이 자동으로 붙는 경우, 본문은 출처와 참고 근거에서 끝내고 JSON-LD만 둔다)
